...
function myblogguest_find_books_page() {
...
echo mbgRunTpl('books_page', array('options' => $options));
}

function mbgFindBooksAjax(){
	...
	$res = mbg_api($url, $body);
	    	...
	    	{
	    	$res['code'] = mbgRunTpl('books_page_table', array('books' => $res['books']));
	    	die (json_encode($res));
	    	}
}
...
add_action( 'wp_ajax_nopriv_mbgFindBooksAjax', 'mbgFindBooksAjax' );
add_action( 'wp_ajax_mbgFindBooksAjax', 'mbgFindBooksAjax' );



// отправка eBook Offer - AJAX Handler

function mbgSendBookOfferAjax()
{
...
$id_site = isset($options['wpmbg_id_site']) ? intval($options['wpmbg_id_site']) : 0;
    ...
    {
    $url = WPMBG_BASE_URL . "/send_book_offer";
...
add_action( 'wp_ajax_nopriv_mbgSendBookOfferAjax', 'mbgSendBookOfferAjax');
add_action( 'wp_ajax_mbgSendBookOfferAjax', 'mbgSendBookOfferAjax');
