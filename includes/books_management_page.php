...
function myblogguest_my_books_page() {
...
echo mbgRunTpl('books_given_to_me', array('options' => $options, 'wpmbg_status' => $wpmbg_status));
}

function mbgBooksGivenToMeAjax(){
	...
	$res = mbg_api($url, $body);
	    	...
	    	{
	    	$res['code'] = mbgRunTpl('books_given_to_me_table', array('books' => $res['books']));
	    	die (json_encode($res));
	    	}
}

// create custom Ajax call for WordPress
add_action( 'wp_ajax_nopriv_mbgBooksGivenToMeAjax', 'mbgBooksGivenToMeAjax');
add_action( 'wp_ajax_mbgBooksGivenToMeAjax', 'mbgBooksGivenToMeAjax');


function mbgImportBookToDraftAjax(){
	// This will obtain the book description using the ID then attempt to publish directly to wordpress
	...
	$url = WPMBG_BASE_URL . "/book_descr_code/$wpmbg_id_descr";
	$descr = mbg_api($url);
	...
	$old_post = mbgGetPostByTitle(trim($descr['title']));
		...
		if(!isset($options['published_books_descr']))
		{
		$options['published_books_descr'] = array();
		}
		
			// Check whether post already exists
		if(!empty($old_post) && !empty($options['published_books_descr'][$wpmbg_id_descr]) && $options['published_books_descr'][$wpmbg_id_descr] == $old_post->ID)
		{
		...
		$post_id = wp_insert_post($new_post);
			...
			if (!empty($post_id))
			{
			$options['published_books_descr'][$wpmbg_id_descr] = $post_id;
...
// create custom Ajax call for WordPress
add_action( 'wp_ajax_nopriv_mbgImportBookToDraftAjax', 'mbgImportBookToDraftAjax' );
add_action( 'wp_ajax_mbgImportBookToDraftAjax', 'mbgImportBookToDraftAjax' );



function mbgBookNotifyMBGAjax(){
	...
	if(isset($options['published_books_descr']))
	{
	$published_books_descr = &$options['published_books_descr'];
	}
	else
	{
	$published_books_descr = array();
	}
	
	if (array_key_exists($wpmbg_id_descr, $published_books_descr))
	{
	$post_id = $published_books_descr[$wpmbg_id_descr];
	}
	...
	$do_ajax = true;
		...
		$save_url = WPMBG_BASE_URL . "/save_books_url";
		...
		$res = mbg_api($save_url, $body);
			...
			unset($published_books_descr[$wpmbg_id_descr]);
			update_option('wpmbg_options', $options);
			}
		}					
	}
...
// create custom Ajax call for WordPress
add_action( 'wp_ajax_nopriv_mbgBookNotifyMBGAjax', 'mbgBookNotifyMBGAjax' );
add_action( 'wp_ajax_mbgBookNotifyMBGAjax', 'mbgBookNotifyMBGAjax' );


function mbgBookRejectAjax(){
	...
	$res = mbg_api($save_url, $body);
		...
		{
		$message ="MyBlogGuest Have Been Informed";
		mbgDelDraftBookDescr($wpmbg_id_descr);
		}
...
// create custom Ajax call for WordPress
add_action( 'wp_ajax_nopriv_mbgBookRejectAjax', 'mbgBookRejectAjax' );
add_action( 'wp_ajax_mbgBookRejectAjax', 'mbgBookRejectAjax' );
